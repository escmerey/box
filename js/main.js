$(document).ready(function() {
    ...
    $(window).on('scroll', function(event) {
        if (isIntoView($('.s_about .h1'))) {
            time = 0;
            $('.s_about__count').each(function(index, el) {
                numberAnimate($(this).find('span'), 0, 700, time);
                time = time + 700;
            });
        };
    });
...
function numberAnimate(numBlock, numStart, time, delay) {
    var count = $(numBlock).parent().data('number');
    setTimeout(function() {
        $({
            count: numStart
        }).animate({
            ...
            // easing: 'easeInSine',
            step: function() {
                numBlock.text(Math.ceil(this.count));
            },
            complete: function() {
                $(numBlock).text(count);
            }
...
function isIntoView(elem) {
    if (elem.length) {
        ...
        if (docViewBottom >= elemBottom && !$(elem).hasClass('animate')) {
